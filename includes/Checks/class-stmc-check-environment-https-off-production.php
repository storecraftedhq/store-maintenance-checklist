...
final class STMC_Check_Environment_Https_Off_Production extends STMC_Check_Base {
	...
	public function title(): string {
		return __( 'Site not served over HTTPS', 'store-maintenance-checklist' );
	}
	...
	public function evaluate( STMC_Check_Snapshot $snapshot ): array {
		...
		return $this->open(
			$snapshot,
			__( 'Checkout and payments risk when the site is not served over HTTPS.', 'store-maintenance-checklist' ),
			array(
				'summary' => sprintf(
					/* translators: %s: home URL */
					__( 'Home URL scheme is not HTTPS (%s).', 'store-maintenance-checklist' ),
					$snapshot->home_url
				),
				'count'   => 1,
			),
