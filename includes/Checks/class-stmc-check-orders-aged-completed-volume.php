...
final class STMC_Check_Orders_Aged_Completed_Volume extends STMC_Check_Base {
	...
	public function title(): string {
		return __( 'Large volume of aged completed orders', 'store-maintenance-checklist' );
	}
	...
	public function evaluate( STMC_Check_Snapshot $snapshot ): array {
		...
		return $this->open(
			$snapshot,
			__( 'Admin and report performance can suffer with large aged completed order volumes.', 'store-maintenance-checklist' ),
			array(
				'summary' => sprintf(
					/* translators: %d: order count */
					__( '%d completed orders older than 24 months.', 'store-maintenance-checklist' ),
					$count
				),
				'count'   => $count,
			),
